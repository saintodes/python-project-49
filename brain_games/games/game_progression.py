...
def get_question_and_answer():
    ...
    first_number = randint(progression_first_digit_random_from,
                           progression_first_digit_random_to)
    numeric_progression.append(first_number)
    progression_step = randint(progression_step_random_from,
                               progression_step_random_to)

    for _ in range(9):  # We add 9 more numbers since the first number is already added
